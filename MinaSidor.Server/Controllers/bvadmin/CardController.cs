...
namespace Cluster.Controllers
    ...
    public class CardController : Controller
        ...
        public CardController()
            {
            aPIClient=  new APIClient("https://billogram.com/api/v2/", "9928-AWbM0oDS", "6831b238067157ab52c550f43bd5fab5");
            }
        ...
        public async Task<IActionResult> ProdukterAsync()
            {
                      CustomerParamQuery querySearchParameter = new CustomerParamQuery
                {
                // Ange sökkriterier och sorteringskriterier om det behövs
                Search_Field = CustomerParamQuery.SearchField.Name,
                Order_Field = CustomerParamQuery.OrderField.Name,
                };
            ItemParamQuery querySearchParameter2 = new ItemParamQuery
                {
                // Ange sökkriterier och sorteringskriterier om det behövs
                Search_Field = ItemParamQuery.SearchField.Item_No,
                Order_Field = ItemParamQuery.OrderField.Price,
                };  
            Billogram.Structures.Item.List itemList = await aPIClient.FetchList<Billogram.Structures.Item.List>(querySearchParameter2);
            var x=itemList.ToJson();
           // Billogram.Structures. customerList = await aPIClient.FetchList<Billogram.Structures.Customer.List>(querySearchParameter);
        ...
        public ActionResult Faktureringsperiod()
            {
            // Logik för att hantera faktureringsperiod
            return View("billing");
            }

        public ActionResult FaktureringsAvtalOchMix()
            {
            // Logik för att hantera faktureringsavtal och mix
            return View("billing");
            }

        public ActionResult MikroAvtal()
            {
            // Logik för att hantera mikroavtal
            return View("billing");
            }
        public async Task<IActionResult> CustomerReportAsync()
            ...
            Billogram.Structures.Customer.List itemList = await aPIClient.FetchList<Billogram.Structures.Customer.List>(querySearchParameter);
            var x= itemList.ToJson();   
            return View(itemList);  
            }
        public async Task<IActionResult> InvoiceReportAsync()
            {
            InvoiceParamQuery querySearchParameter2 = new InvoiceParamQuery
                {
                Order_Field = InvoiceParamQuery.OrderField.Customer_Name,
                };
            Billogram.Structures.Invoice.List itemList = await aPIClient.FetchList<Billogram.Structures.Invoice.List>(querySearchParameter2);
